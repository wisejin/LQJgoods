...
@interface NSString (RichTextReplyInputView)
@end

@implementation NSString (RichTextReplyInputView)
...
@implementation RichTextReplyInputView
...
- (void) composeView
{
  ...
  textView = [[UITextView alloc] initWithFrame:CGRectMake(70.0f, topGap, 185 + SCREEN_WIDTH - 320, 0)];
...
- (void)layoutSubviews
{
  ...
  //最上面的一条细线
  UILabel *dividLbl = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 1)];
  ...
  textView.frame = CGRectMake(5, textView.frame.origin.y, SCREEN_WIDTH - 10 - 65, textView.frame.size.height);
...
- (id) initWithFrame:(CGRect)frame andAboveView:(UIView *)bgView
{
  ...
  if (self)
  {
    tapView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, SCREEN_HEIGHT)];
...
- (void) sendButtonPressed:(id)sender
{
  ...
  [_delegate replyInputWithReply:textView.text appendTag:_replyTag];
  [self disappear];
