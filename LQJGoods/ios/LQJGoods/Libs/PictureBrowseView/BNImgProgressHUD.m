...
@implementation BNImgProgressHUD

+ (instancetype)showHUDto:(UIView *)view animated:(BOOL)animated
{
    BNImgProgressHUD *hud = [[self alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
    return hud;
}
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated
{
    ...
    {
        if ([aView isKindOfClass:[BNImgProgressHUD class]])
        {
            [huds addObject:aView];
        }
    }
    
    for (BNImgProgressHUD *hud in huds)
    {
        [hud hide:animated];
    }
    return [huds count];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        //比例显示
        _ratioLabel = [[UILabel alloc] initWithFrame:CGRectMake(_rotateView.frame.origin.x-2.5, _rotateView.frame.origin.y, _rotateView.frame.size.width+5, _rotateView.frame.size.height)];
        _ratioLabel.font = [UIFont boldSystemFontOfSize:8];
        _ratioLabel.textAlignment = NSTextAlignmentCenter;
        _ratioLabel.text = @"0%";
        _ratioLabel.textColor = [UIColor whiteColor];
        [_toast addSubview:_ratioLabel];
